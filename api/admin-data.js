...
function authUrl(path) {
  return SECRET ? `${DB}/${path}.json?auth=${SECRET}` : `${DB}/${path}.json`;
}
...
async function fbSet(path, data) {
  try {
    await fetch(authUrl(path), { method:"PUT", headers:{"Content-Type":"application/json"}, body:JSON.stringify(data) });
    return true;
  } catch(e) { return false; }
}
async function fbPush(path, data) {
  try {
    const r = await fetch(authUrl(path), { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(data) });
    const j = await r.json();
    return j.name || null;
  } catch(e) { return null; }
}
async function fbDel(path) {
  try { await fetch(authUrl(path), { method:"DELETE" }); return true; }
  catch(e) { return false; }
}
function cleanPhone(ph) {
  if (!ph) return "";
  if (typeof ph === "object") return String(ph.number || ph.phone || Object.values(ph)[0] || "");
  return String(ph);
}
...
export default async function handler(req, res) {
  ...
  if (!DB) return res.status(500).json({ ok: false, msg: "FIREBASE_DB_URL missing" });
  if (!ADMIN_KEY) return res.status(500).json({ ok: false, msg: "ADMIN_KEY missing" });
  ...
  try {
    if (action === "getData") {
      ...
      return res.json({ ok: true, tools: tools||null, keys, phone: cleanPhone(phone), settings: settings||{}, views: parseInt(views)||0 });
    }

    if (action === "addTool") {
      const { name, link, desc, iconClass, iconUrl, iconBase64 } = payload;
      if (!name || !link) return res.json({ ok: false, msg: "Name & Link required" });
      const id = await fbPush("tools", { name, link, desc:desc||"", iconClass:iconClass||"fas fa-star", iconUrl:iconUrl||null, iconBase64:iconBase64||null, active:true, created:Date.now(), views:0 });
    ...
    if (action === "generateKey") {
      ...
      let kv;
      if (custom) {
        kv = custom.toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,20) || "LH-CUSTOM";
      } else {
        kv = "LH-";
        for (let i=0; i<4; i++) {
          let s="";
      ...
      const id = await fbPush("keys", { key:kv, days, expiry:Date.now()+days*86400000, created:Date.now(), used:false, usedBy:null, deviceId:null });
    ...
    if (action === "addScreenshot") {
      ...
      const id = await fbPush("screenshots", { url:payload.url, caption:payload.caption||"", order, created:Date.now() });
      return res.json({ ok: !!id, id });
    }
    if (action === "getScreenshots") {
      ...
      const arr = [];
      for (const id in shots) { if (shots[id] && shots[id].url) arr.push({ id, ...shots[id] }); }
